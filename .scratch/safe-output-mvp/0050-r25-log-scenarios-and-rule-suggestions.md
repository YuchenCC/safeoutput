# R2.5 日志场景与规则建议演示

> *This was generated by AI during triage.*

- category: enhancement
- state: ready-for-agent
- source: doc/prd/safe-output-r25-prd.md
- wbs: R2.5-04
- blocked_by: 0047-r25-business-workbench-and-mock-domain.md

## What to build

在 Demo 中增加日志场景演示页和触发接口，通过真实 Log4j2 logger 输出触发 `%safeOutputMsg`。场景应覆盖 JSON-like 日志、key=value 日志、regex fallback、已配置 key 和未配置 key，并在页面展示触发后的 LOG 统计和日志规则建议变化。

本切片不得读取、展示或导出原始业务日志文件内容。页面可以展示预设日志模板、脱敏预期、统计结果和规则建议。

## Acceptance criteria

- [ ] 日志场景通过真实 Log4j2 logger 触发，不在 controller 中手工 seed 规则建议。
- [ ] 支持触发 JSON-like 日志样例。
- [ ] 支持触发 key=value 日志样例。
- [ ] 支持触发 regex fallback 日志样例。
- [ ] 支持触发已配置 key 和未配置 key 的对比样例。
- [ ] 页面展示 LOG 脱敏计数或相关聚合指标变化。
- [ ] 页面展示日志规则建议列表和配置片段。
- [ ] 日志演示不读取原始日志文件，不展示完整原始 message。
- [ ] 日志演示继续遵守不保存敏感原文的报告边界。

## Blocked by

- 0047-r25-business-workbench-and-mock-domain.md

## Test requirements

- 覆盖日志触发接口的 HTTP 集成测试。
- 验证触发后 LOG 场景统计可观察。
- 验证未配置 key 能进入日志规则建议。
- 验证接口不返回完整原始日志 message。
- 运行 `mvn -pl safe-output-demo -am test`。
