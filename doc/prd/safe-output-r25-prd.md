...
2. 接入方式展示不完整，YAML、注解、默认规则、ignore、Log4j2 PatternConverter 和主动脱敏服务虽然已经存在，但缺少一个能把“业务场景”和“接入选择”对应起来的说明页面。
3. 脱敏实验能力较分散，当前能验证主动脱敏和幂等，但还不能系统展示不同类型、强文本扫描、对象脱敏、批量性能等场景。
4. 报告能力已有 JSON 快照，但 Demo 还不能像报告中心一样列出存储目录下的报告文件、查看单个报告并可视化展示风险、Top 排名、遗漏建议和性能指标。
...
Demo 应模拟一个已经接入 Safe Output 的业务系统。该系统拥有自己的 mock 数据源、业务接口、日志输出、主动脱敏工具、报告中心和前端控制台。Safe Output 作为这个业务系统的输出侧脱敏能力存在，而不是页面上的孤立工具按钮。
...
4. 通过接入说明页面展示 YAML、注解、默认规则、ignore、Log4j2 PatternConverter 和 `SafeOutputMaskService` 的适用场景。
5. 通过脱敏实验室展示按类型脱敏、对象主动脱敏、强文本扫描、幂等验证和基础性能测试。
6. 通过日志场景页面触发 JSON-like、key-value、regex fallback、已配置 key 和未配置 key 的日志样例，并展示统计和规则建议变化。
7. 通过报告中心导出脱敏统计 JSON 报告、列出当前报告目录中的 JSON 文件、读取单个报告并渲染可视化报表。
...
14. As a 接入方, I want 看到 Log4j2 `%safeOutputMsg` 的真实触发入口, so that 我知道日志脱敏不是 controller 手写替换。
...
24. As a Demo 使用者, I want 执行批量性能测试, so that 我能观察不同输入规模下的平均耗时。
25. As a Demo 使用者, I want 触发不同日志样例, so that 我能看到 JSON-like、key-value 和 fallback 的差异。
...
28. As a Demo 使用者, I want 从报告页面打印或导出 PDF, so that 我可以把报告用于演示和评审材料。
...
34. As a 评审者, I want 每个演示场景都能从页面直接触发, so that 展示过程不依赖命令行或人工解释源码。
...
- 接入方式说明应以“业务场景 -> 接入方式 -> 示例接口 -> 字段或日志 key -> 规则来源 -> 输出效果”为核心结构，不做长篇静态说明。
- Response 演示必须保留 fail-open、安全统计和 ignore 边界，不通过 Demo 手写脱敏结果替代真实 `ResponseBodyAdvice` 行为。
- 日志演示必须通过真实 Log4j2 logger 输出触发 `%safeOutputMsg`，不在 Demo controller 中手工 seed 日志建议。
- 主动脱敏实验室继续使用 `SafeOutputMaskService`，并扩展按类型、对象、强文本、幂等和批量性能场景。
- 报告中心新增 Demo 侧报告文件浏览能力，读取 `safe-output.report.directory` 下的 JSON 报告文件，展示文件名、大小、修改时间和报告数量。
...
- Demo 页面应优先成为一个业务系统控制台，默认首屏展示业务工作台和风险摘要；工具型能力放入实验室和报告中心。
...
- 日志演示测试需要验证触发接口能产生 LOG 场景统计和日志规则建议，不要求读取原始日志文件。
- 实验室测试需要覆盖按类型脱敏、对象主动脱敏、强文本扫描、连续脱敏幂等字段和批量性能结果结构。
- 报告中心测试需要覆盖导出 JSON、列出报告文件、读取单个报告、单报告 dashboard 数据，以及非法文件名或路径穿越输入被拒绝。
- 前端验收以人工运行 Demo 为主，确认业务工作台、接入说明、日志场景、脱敏实验室和报告中心可以从页面触发完整流程。
...
- PDF 展示需求优先通过浏览器打印满足，待 HTML 报告结构稳定后，再考虑是否在后续版本引入服务端 PDF 渲染。
