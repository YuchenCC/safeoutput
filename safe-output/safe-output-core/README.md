...
规则命中但未找到对应策略时默认跳过当前字段，不回退到 `DEFAULT`；核心会记录 warning，并通过 `UnknownTypeRecorder` 暴露聚合统计钩子。成功脱敏会通过 `MaskEventRecorder` 记录场景、String 类型标签和耗时，便于内置类型和自定义类型共用同一统计链路。主动脱敏统一记录为 `MANUAL` 场景，不写入响应接口风险统计。
