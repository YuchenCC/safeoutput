...
- 报告导出、Demo Dashboard 和 Demo Log 规则建议接口会复用 `safe-output.rules[].keys` 过滤已配置 key，避免重复治理建议；过滤只影响报告建议，不影响 Log4j2 在线脱敏和聚合计数；未过滤的低置信度建议也会生成 `enabled:false` YAML 候选片段供人工复核。
